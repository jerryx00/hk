<?php
    ...
    * CZ接口
    */
    class TxorderController extends BaseorderController {

        protected $ORDER_POST_URL = '/Txauth/order?';
        protected $ORDER_NOTIFY_URL = '/Txauth/notify';

        protected $ORDER_MODEL = 'order_tx';
        protected $GOODS_MODEL = 'goods_tx';
        ...
        public function insert() { 
            ...
            $goodList = M($this->GOODS_MODEL)->where(['fluxnum' => $d['fluxnum'], 'status'=>'1'])->find();

            if ($goodList) {
                $d['fluxid'] = $goodList['fluxid'];
            } else {
                //return $this->errorReturn('该商品不存在,请重新提交！', $reloadUrl);
                $retContent['header']['errcode'] = '1000999';
                $retContent['header']['errmsg'] = '该商品不存在,请重新提交！';
            ...
            if ($ret === false) {  
                //                return $this->errorReturn('输入错误,请重新提交！', $reloadUrl);
                $retContent['header']['errcode'] = '1000999';
                $retContent['header']['errmsg'] = '输入错误,请重新提交！';
                $this->ajaxReturn($retContent);
            }
